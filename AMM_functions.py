...
def simulation(M_, N_, T_, dt_, buy_, sell_, eta0_, eta1_, S0_, X_, Y_, filtr_bfs, filtr_sfs, seed=42):
    ...
    Returns:
        A numpy array with the following metrics:
            [avg_buy_fee_rev, avg_sell_fee_rev, avg_arb_fee_rev, avg_imp_loss]
            where:
              ...
              - avg_imp_loss    : Average impermanent loss.
    """
    # Set random seed for reproducibility
    np.random.seed(seed)
    ...
    # Initialize the marginal price at time 0 and store initial reserves
    ...
    for i in range(1, N + 1):
        # Obtain current CEX price for all indices at time i
    ...
    # Compute average fee revenues and impermanent loss over the simulation horizon
