...
@njit
def arb_trade_single(S, X, Y, etaA, eta1):
    """
    ...
    Parameters:
        ...
        etaA : CEX proportional cost for arbitrageur.
    ...
    val1 = Y * (1 - np.sqrt((P * (1 - eta1)) / (S * (1 + etaA))))
    if val1 > 0:
        q1 = 0.0
    else:
        q1 = val1
    
    val2 = Y * (1 - np.sqrt((P * (1 + eta1)) / (S * (1 - etaA))))
    if val2 < 0:
        q2 = 0.0
    else:
        q2 = val2
...
@njit
def fast_simulation(M, N, T, dt, buy_amt, sell_amt, eta0, eta1, etaA, S0, X0, Y0, filtr_bfs, filtr_sfs):
    """
    ...
    for i in range(1, N+1):
        ...
        for j in range(M):
            ...
            D_A = arb_trade_single(S, X_prev, Y_prev, etaA, eta1)
    ...
    return Pool_X, Pool_Y, S0, S1, CPMM_buy_revenue, CPMM_sell_revenue, CPMM_arb_revenue, Hedging_port_val, CPMM_buy, CPMM_sell, Arb
...
@njit
def fast_simulation_summary(M, N, T, dt, buy_amt, sell_amt, eta0, eta1, etaA, S0, X0, Y0, filtr_bfs, filtr_sfs):
    """
    ...
    for j in range(M):
        for i in range(1, N+1):
            ...
            avg_buy_vol += CPMM_buy[i, j]
            avg_sell_vol += CPMM_sell[i, j]
            avg_arb_vol += np.abs(Arb[i, j])
    avg_buy_fee_rev /= M
    avg_sell_fee_rev /= M
    avg_arb_fee_rev /= M
    avg_buy_vol /= M
    ...
    result[5] = avg_buy_vol
    result[6] = avg_sell_vol
    result[7] = avg_arb_vol
